...
pub(crate) const OPTIONS_MIN: usize = 2;
pub(crate) const OPTIONS_ADOPTED: usize = 1;
pub(crate) const RETREAT: Keys = Keys {
...
pub(crate) const APPROVAL: Keys = Keys {
...
pub(crate) const FLOOR: Floor = Floor::Map(&[
    ...
    (
        "enums_note",
        Floor::Val(
            "retreat_kind は組み立て時に憲法 schema.enums.retreat_kind から導出した列（便 49）。置き場の憲法の retreat_kind にこの列に無い値が在れば「まだ分からない」（部分集合の比較・順と重複は問わない・FR25・便 122）。surface は対話面を rules 行の id で指す（P-5.2・P-12.1 が名指す R-8 = 持ち主と orchestrator 席の対話面）",
        ),
    ),
    (
        "status_note",
    ...
    (
        ...
        Floor::Strs(&[
            ...
            "anchor に在って現行に無い条は落とす（番号は消さない・P-7。条の廃止の機構は無い＝limits_note の条の廃止（status）の項）。規範文 id の重複・改番・廃止した番号の再利用・印を値に持つ欄も落とす",
    ...
    (
        ...
        Floor::Strs(&[
            ...
            "凍結と検査は同じ写しの関数を通る（生成物どうしの突き合わせ・P-10.2）。anchor は差分検査の比較元であって P-10.1 の「独立した凍結 anchor」ではない。P-10.1 の機構の状態は憲法の条 P-10 の機構の欄が持つ（ここには写さない）。tests/floor_cases.yaml は歯の入力で、床の入力ではない（fixture の不在は床では検出されず、歯 crates/folio/tests/floor_cases.rs が落とす・判断の記録 ADR-2 の注）。",
            ...
            "rules 行（rules.yaml）は本 anchor の外。P-17.2（裁定 id の無い rules 行の変更を落とす）と P-17.4（deny 行を緩めるときは A-2.3 を前置）の機構と今の状態は憲法の条 P-17 の機構の欄が持つ（ここには写さない）。対話面 R-8 の中身（rules 行の what）も同じく anchor の外＝床は対話面を rules 行の id の実在と値域（surface）で見るだけで、行の本文の書き換えは止めない（P-17 の機構の領分）。",
            ...
            "条の廃止（status）の機構は憲法 schema に無い（条の欄に status が無く、足すと床が未知の欄で落とす・便 128）。条は消せず、廃止も表せない＝P-7.2「廃止は状態で」を条に適用する形は M0 では決めておらず、台帳 f2-648.100 に起票した（判断の記録 ADR-2 の帰結）。",
