...
/// fixture の正本 4 file と index.yaml・intake.yaml・adr/ADR-2.yaml を一時 dir の下の src/ へ写す
/// （支度表 intake-sheet.yaml と期待の面は写さない）。写す判断の記録は欄の揃った便 25 の 1 本（便 26 §1 (d)）。
...
fn face_index_shelf_adr_card_is_absent_without_records() {
    ...
    assert!(card.contains("sc-none"), "{card}");
    assert_eq!(readable_faces(&html), "3");
}
...
fn face_index_census_on_the_real_sources_counts_and_verbatims() {
    ...
    assert_eq!(readable_faces(&html), (3 + adr).to_string(), "読める面の数");
