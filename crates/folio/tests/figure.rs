...
/// 凍結の写し（道具の生の出力）に便 67 と便 82 の置き換えを当てた byte 列 = folio が書く図の本体。
/// 置き換えは凡例の見出しの要素の中身・1 つ目の開始タグの言語の宣言（便 67）と、構成図の説明・読み上げの種別・
/// 読み上げの頭（便 82）だけで、他の byte は動かない（docs/design/delivery-67.md §1 (c) 4・delivery-82.md §1 (b)・
/// 置き換えの字面は歯の側で持ち、生成側の関数を呼ばない）。
fn frozen_body_ja() -> Vec<u8> {
    ...
    assert_eq!(raw.matches(desc).count(), 1, "凍結の写しに説明が 1 つでない");
    let roles = raw.matches(role).count();
    let focuses = raw.matches(focus).count();
    assert!(roles > 0 && focuses > 0, "凍結の写しに読み上げの英語が無い");
    let swapped = raw
        .replacen("lang=\"en\"", "lang=\"ja\"", 1)
        .replacen(">Legend<", ">凡例<", 1)
        .replace(desc, "図の道具（Archify）が描いた構成図。")
        .replace(role, "構成図の要素")
        .replace(focus, "aria-label=\"注目 ");
    let delta = |a: &str, b: &str, n: usize| (b.len() as isize - a.len() as isize) * n as isize;
    // 便 67 の置き換えは byte 数を変えない・便 82 の置き換えの分だけ動く
    assert_eq!(
        swapped.len() as isize - raw.len() as isize,
        delta(desc, "図の道具（Archify）が描いた構成図。", 1)
            + delta(role, "構成図の要素", roles)
            + delta(focus, "aria-label=\"注目 ", focuses),
        "置き換えの外で byte 数が動いた"
    );
    swapped.into_bytes()
}
...
fn figure_write_matches_the_frozen_anchor() {
    ...
    assert_eq!(stdout(&run), "folio figure: 書いた（16731 byte）\n");
}
...
// ── 11. 図の本体の型ごとの英文の日本語（便 82・docs/design/delivery-82.md §1 (c)・天井の 18 周目の読みやすさ F-5）──

#[test]
fn f82_architecture_figure_has_no_english() {
    let (run, body) = anchor_write("f82-arch");
    assert_eq!(code(&run, "folio figure --write"), 0, "{}", stderr(&run));
    for en in [
        "An architecture diagram generated by Archify.",
        "Architecture component",
        ...
        assert_eq!(body.matches(en).count(), 0, "英語「{en}」が残っている");
    }
    for ja in [
        "図の道具（Archify）が描いた構成図。",
        "構成図の要素",
        "注目 ",
    ] {
        assert!(body.contains(ja), "日本語「{ja}」が無い");
    }
}

#[test]
fn f82_workflow_and_sequence_figures_have_no_english() {
    let td = temp_dir("f82-real");
    let mut bodies = String::new();
    for id in ["fig-plain", "fig-tech"] {
        let out = td.join(format!("{id}.svg"));
        let run = folio_figure("figures", id, &design_intent(), &out, "--write");
        assert_eq!(code(&run, id), 0, "{id}: {}", stderr(&run));
        bodies.push_str(&fs::read_to_string(&out).unwrap_or_default());
    }
    let _ = fs::remove_dir_all(&td);
    for en in [
        ...
        assert_eq!(bodies.matches(en).count(), 0, "英語「{en}」が残っている");
    }
    // 実の図 2 枚は説明（desc）を型付き記述で持つので道具の英文の説明は出ない（実測 2026-09-22）。
    // 読み上げの頭は 2 枚とも置き換わる
    assert_eq!(
        bodies.matches("aria-label=\"Focus").count(),
        0,
        "読み上げの頭の Focus が残っている"
    );
    assert!(bodies.contains("aria-label=\"注目 "), "読み上げの頭が日本語でない");
}

#[test]
fn f82_the_anchor_is_compared_before_the_replacement() {
    // 写しは道具の生の出力のまま（英語の説明が残る）で、それでも照合は緑
    let raw = String::from_utf8(frozen_body()).expect("凍結の写しが UTF-8 でない");
    assert!(
        raw.contains("An architecture diagram generated by Archify."),
        "凍結 anchor の写しから英語の説明が消えている（写しを書き換えた）"
    );
    let (run, body) = anchor_write("f82-anchor");
    assert_eq!(code(&run, "folio figure --write"), 0, "{}", stderr(&run));
    assert!(
        !stderr(&run).contains(ANCHOR_DRIFT),
        "置き換えの後の値で照合している: {}",
    ...
    assert!(
        !body.contains("An architecture diagram generated by Archify."),
        "書いた図の本体に英語の説明が残っている"
    );
}
