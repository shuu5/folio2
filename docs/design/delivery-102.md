...
| crates/folio/tests/graph.rs（凍結の要約値 1 つ） | 1 |
...
| 合計 | **352**（版管理が数える生の行では足す 363・消える 84） |
...
- `crates/folio/src/` の ceiling.rs と bundle.rs 以外（schema.rs・check.rs・graph.rs・stamp.rs・gate.rs・findings.rs・面の生成器）。**床の判定も天井の印も門の 3 値も 1 つも変わらない。**
