# 設計: 便 107 — 正本の cursor（結果の型 R・1 file の読み load・木を辿る X・字面の逃がし esc・id の形 safe_id）を `face.rs` から新しい `cursor.rs` へ責務で降ろし、層が上がる辺を 13 本から 8 本へ減らす（参考値・base main b8fb27f）（振る舞い不変・FR4 / FR1 / FR20）
...
- 改訂: 改訂 b（2026-09-23・検証役 d107-verify.md の直し 1〜4）= §1 (h) 4 の穴の記述を変異の実測 16 / 27 に合わせ、審査が (i) 3 で区間の純度を確かめる形に・§4 に 1 文・(b) に esc の 1 文・参考値の断り。数値・write-set・verify・done・size は直していない。
...
2. **正本の cursor**（取り出しの結果の型 R・正本 1 file を型付きで読む load・型付きの木を欄の道つきで辿る X とその関連 fn・字面の 5 字の逃がし esc・id の形を確かめる safe_id）。正本の byte を型に直して辿るだけで、HTML の骨格も名札も知らない。ただし字面の逃がし esc は HTML の 5 字を逃がすので、見た目の知識の最小の一片を層 1 へ持ち込む。設計ノート §4-a の b-1-1 のとおり、esc を面の file に残すと上向きの辺が 9 本になるので、区間に含める。責務の層は **1 読む**。
...
4. **X の関連 fn の断りの側は、ほとんどどの歯も測っていない。** 検証役が区間に 27 の変異を入れたところ、次の 16 は workspace の 779 本（参考値・base main b8fb27f）が全部緑だった。字面の fn が真偽と null を断ること、その断りの文に欄の道を付けること。id の fn が id の形を確かめること、形と型の断りの文に欄の道を付けること。必須の欄の fn が表でない点と無い欄を断ること、その文に道を付けること。任意の欄の fn が null を無いとみなすこと。一覧の要素の道の添字。表のキーが文字列であること。整数の fn が文字列を断ること。機械のための字面の fn が null・一覧・逃がしを扱うこと。load の読めない断りに file 名を付けること。測っているのは、字面の逃がし・id の形の関数そのもの・重複キー・子の点の道・名札と値域の表引きの断りである（検証役の記録 d107-verify.md §4）。本便は区間を字を変えずに移すので振る舞いは変わらない。ただしその主張を支えるのは歯ではなく、(b) の区間の突き合わせと (f) の出力の byte 比較である。**着地の時にこれを確かめられるのは取り込みの審査だけである。審査は (i) の 3 の手順で区間の純度を確かめる（違いは child の見え方の 1 語だけのはず）。** 穴を塞ぐ歯を足すのは本便の外（後続の候補・検証役が最小の形を d107-verify-tooth.patch に試作した）。
...
§1 (e) のとおり。本便は歯を足さず、既に在る歯は 1 本も名を変えず 1 本も消さず 1 本も置き場を移さない。境界の歯 2 本は型付きデータだけが変わる。区間の変化を捉える歯の範囲と限界は §1 (d) と §1 (h) の 4・5 に書いた。verify の 4 行で落ちない変化があるので、共通の検証の `.vessel.toml` の common-verify（workspace 全体の nextest と clippy）を合わせて判定する。**common-verify でも落ちない変化がある**（§1 (h) 4）ので、取り込みの審査は区間の純度を §1 (i) の 3 の手順で確かめる。
