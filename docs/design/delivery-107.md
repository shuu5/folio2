...
- 前の便: **便 106 着地済み（main b8fb27f・床の機械を `floor.rs` へ・行 `de`・台帳 f2-648.151）**。便 107 が触る src のうち便 106 が触ったのは入口 `main.rs`（区切りの宣言 1 行）だけで、境界の歯の file `crates/folio/tests/modules.rs` は便 106 の実物の凍結一覧（13 対）に当てた。起草役は初めに main a358be9 と便 106 の起草役の実測 patch の木で測り始めたが、着地を受けて写しを b8fb27f の実物に取り直し、区間・write-set・余地・上向きの辺・歯の本数・見え方の拡大・出力の byte をすべて測り直した。この契約の数はすべて b8fb27f の実測（参考値）である。ほかの前提は便 105（歯の `tests/face.rs` の割り・着地済み）。**base は main b8fb27f**。
