...
(a) 読み: `crates/folio/src/check.rs` の正本の一覧の定数を 4 本から 5 本（憲法・rules・語彙・要件書・入口の順）にし、入口の正本 index.yaml を他の 4 本と同じ読み手（同じ関数）で読む。したがって index.yaml が symlink・無い・file でない・読めない・parse できない・最上位が欄の表でない はどれも他の 4 本と同じ文言の形で「まだ分からない」（終了 2）になり、無いときの文言は「index.yaml: 正本が無い」。重複キーは他の 4 本と同じ種別「重複キー」の違反。5 本のどれかが読めなければ、今と同じく後段の検査には進まない。今の読みは一覧の順に読んだ結果を末尾から 4 回取り出して 4 本に割り当てているので、一覧を 5 本にするときは取り出しも 5 本に合わせる（入口を先に取り出すか、名で取る）＝憲法・rules・語彙・要件書の割り当てをずらさない。参照 id・語彙 R-9・判断の記録との突き合わせ・凍結 anchor・読み物の生成の各検査は、今と同じく憲法・rules・語彙・要件書の 4 本だけを引数で受ける（一覧の定数を直接は読まない・admin 席の実測）ので、これらの母集団に index.yaml は入らない。
