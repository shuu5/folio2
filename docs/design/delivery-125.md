...
- 門: 本便は設計文書の正本（design-intent の下）を 1 本も書き換えない。起草役が write-set 9 本を base の binary の `folio ceiling --gate --write-set …` に渡すと **0（通す・設計文書の正本を書き換えない便）**（2026-09-24・base c33ee44）。受付の時点の main で撃ち直す。
...
**1. 生成区間（9 file）。** `crates/folio/src/schema.rs` の表の 9 file の各床の木を、`folio schema --write` と同じ導出（床の導出の関数と印 2 本）で書く。表を init.rs に写さず、schema.rs の表をそのまま引く（同じ一覧を 2 か所に持たない・P-6.3）。区間の位置は folio2 の正本と同じ（規則の表は先頭の注釈の次・判断の記録と設計ノートの欄の決まりは meta の次・ほかは末尾）。書いた直後の `folio schema --dir <置き場> --check` が 0 になることを歯 1 が見る。**便 ② の後の形（便 ②〔行 `dt`〕の起草役の返事・契約 §5・014a379）。** 便 ② は `crates/folio/src/floor.rs` に、床の木と置き場の憲法の名（無しも受ける）を受けて導出の字を返す関数を足し（名が無しなら今の導出と同じ字）、`crates/folio/src/adr.rs` に置き場の憲法の正本の meta.id を読む関数を足し、`folio schema` は判断の記録の欄の決まり（adr/schema.yaml）のときだけ名を渡す。本便の骨格は、判断の記録の欄の決まりの生成区間をこの名を受ける導出の関数に骨格の憲法の名（雛形の meta.id の定数 floor-declaration）を渡して書き、ほかの 8 file は名無しで書く（`folio schema --write` と同じ振り分け）。表に行の無い名なので、写しの列の根の表は空の表の 1 行になる（便 ② の起草役の返事）。名を読む関数を init が呼ばずに定数を渡してよいのは、書いた直後の `folio schema --check`（名を読む関数で読み直す）が 0 になることを歯 1 が見るからである。どちらの関数も便 ② の新設で、本便の write-set に入れない（本文を変えない）。
...
設計文書の正本を書き換えない便なので、門は 0（通す）で、起草役の実測も 0（§0 の 門）。受付の時点の main で write-set 9 本を `folio ceiling --gate --write-set` に渡して撃ち直す。
...
6. **撤退条件。** (1) 受付の時点の main で、骨格の試作の床が違反 0・まだ分からない 2（凍結の基準の不在だけ）に届かなければ、足りない字を契約の改訂で足してから運ぶ（雛形の字を実装役が考えない）。(2) 受付の時点の main で、便 ② の名を受ける導出の関数と名を読む関数の形が §1 (c) の 1 と違えば（名前・引数・名が無しのときの字）、導出の口の名指しを契約の改訂で直してから運ぶ。(3) 本便の後に (f) の 3 本のほかに既存の歯が 1 本でも落ちたら、その歯の本文を直さずに止めて席へ返す。(4) 利用者の持ち主の裁定（(i) の 3）が「骨格の命令が条を 2 本書く」に決まったら、雛形の定数と歯 5 の期待を直す改訂を先に起こす。
...
- 並行の便: 便 ②（行 `dt`）は `crates/folio/src/main.rs` の命令 check に旗 1 つと振り分けの 1 枝を足し、`crates/folio/src/schema.rs` の命令の本体（1 file を掛ける関数と入口）を名を渡す形に変える（表は変えない）。本便の main.rs の変更は命令 Init の宣言と振り分けの枝と mod の 1 行だけ、schema.rs の変更は表の見え方の 1 行だけで、hunk は重ならないが file は重なるので、便 ② の着地の後に受付ける。便 ③（行 `du`）は main.rs も schema.rs も触らない。便 123 は main.rs の命令 derive の説明の 1 行を変える。
