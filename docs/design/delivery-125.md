...
- 前の便: **base = main 8066e4c（改訂 c で rebase した）。crates・tests・contracts は 2d90e8d から 1 字も変わらないので、この契約の数は base 2d90e8d の実測（参考値）をそのまま使い、受付の時点の main で数え直す**（規則の表の行 D-13）。**受付の順序 = 便 ②（`docs/design/delivery-121.md`〔と 124〕・列の根の表と始まりの凍結）→ 便 ③（`docs/design/delivery-122.md`・値域の部分集合の床）→ 便 123 → 一括 16 → 本便**（ADR-16 決定 (7) の ② → ③ → ④）。**一括 16（要件書 第 1.35 版・FR22 の規範文の書く file の列挙に graph.yaml を足した版）は発効済みで、本改訂 d で本文の FR22 の引用（§1 (b) の 書く file と §1 (d) の 1）をその字に揃えた。**骨格の生成区間は既存の `folio schema --write` の口（schema.rs の run）を呼んで書くので、便 ② が判断の記録の欄の決まりの生成区間を置き場の憲法の名で決まる形にすると、骨格もそのまま同じ形になる（§1 (c) の 1）。
- 分割: ADR-16 決定 (7) の ④「1〜2 便」の 2 便目。本便 = 骨格の命令と案内の 1 行。案内の 1 行を骨格の命令と同じ便に置くのは、案内がまだ無い命令を名指す期間を作らないためである（便 123 の §0 の 分割）。
- 改訂 e（2026-09-24・審査 FAIL goal-done-contradiction への直し）: 歯 5 と done の入口・天井・相談窓口の値の比べ方を、folio2 の値に同じ括弧の落としを歯の側で自前に当てた字と比べる形に改め、§1 (c) の 3 の ウ の 3 の例と §1 (i) の 4 を揃えた（title・write-set・verify は不変）。
- 改訂 d（2026-09-24・FR22 の引用を第 1.35 版に）: 一括 16 の発効を受け、§0 の 前の便・§1 (b)・§1 (d) の 1・§5 の FR22 の引用を第 1.35 版の規範文の字に揃えた（write-set・verify・done は不変）。
...
- 書く file は次の **11 本**（置き場からの相対・この順に書く）: constitution.yaml・rules.yaml・vocabulary.yaml・srs.yaml・index.yaml・intake.yaml・ceiling.yaml（正本 7 file）、graph.yaml（索引の欄の決まり・要件 FR22 の規範文〔第 1.35 版〕の字）、adr/schema.yaml と adr/ADR-1.yaml（判断の記録の置き場と欄の決まりと最初の判断の記録）、design-note/schema.yaml（設計ノートの置き場と欄の決まり）。置き場と adr/ と design-note/ の dir が無ければ作る（置き場の親 dir が無ければ作らずに 2）。
...
**3. 組み立て時に folio2 の正本から読む字（build の時に binary へ焼く・実行時に folio2 の repo を読まない）。** 読む file は folio2 の `design-intent/` の下の constitution.yaml・rules.yaml・index.yaml・ceiling.yaml・intake.yaml の 5 本（組み立て時に字として焼く・crate の外の file を焼くのは憲法の値域を焼く `crates/folio/build.rs` と同じ向き）。meta（承認欄を含む）と schema の生成区間と注釈は、どの file からも写さない。支度表（intake-sheet.yaml）は読まない。
   ...
   - **ウ 入口・天井・相談窓口の節（席の裁定 1）。** この 3 本は字のまま行で写す（相談窓口の鍵 yes と no が型付きでは往復できない・(a) の 8）。式は次の順で、決定的に当てる。
     ...
     3. **folio2 固有の id を持つ括弧を落とす。** 写した字のうち、全角の括弧（ と ）で囲まれた部分（入れ子は数える）の中に、歯 7 の id の形（§1 (e) の 7）で骨格自身の番号（ADR-1・P-1・P-1.1・R-8・R-16）でないものが 1 つでも在れば、その括弧を中身ごと落とす（例 天井の忠実さの観点の問いの末尾の括弧。相談窓口の sheet の括弧 FR2 は sections の中に在り、手順 2 で sections ごと写さないので、この式には掛からない）。この式は字の値の全部（入口・天井・相談窓口の写す節の字）に当てる。
...
1. **graph.yaml を書かない（正本 7 file + 2 置き場だけ）。** `folio schema --check` と schema.rs の run は 9 file が全部無いと合格を返さず、骨格の生成区間を埋める口も確かめる口も止まる。要件 FR22 の規範文（第 1.35 版）が書く file に「索引の欄の決まりの file（graph.yaml）」を数え上げている。採らない。
...
5. **雛形の出所が folio2 の正本と揃う。** 骨格の憲法の schema の値が folio2 の憲法の schema の値と等しく、骨格の憲法の schema の節の字に # が 0 個（注釈の落とし損ねを拾う）。骨格の規則の表の行 R-16 の what と value が folio2 の行 R-16 と値で等しい。入口・天井・相談窓口の写す節の値は、**folio2 の値に §1 (c) の 3 の ウ の 3 と同じ括弧の落としを歯の側で自前に当てた字**と比べる（歯は init.rs の関数を呼ばず、歯の file に同じ式を書く＝実装と歯が同じ関数を使わない。括弧に folio2 の id を持たない値は落としの前後で同じ字）。その比べ方で、骨格の相談窓口の answers と questions の値が folio2 と等しく、targets の各行の id・type・with が folio2 と等しく、sheet が file・title・explain の 3 欄だけで、file と title は folio2 と等しく、explain は folio2 の explain に括弧の落としを当てた字と等しい。骨格の天井の documents の各行の id と file の組が folio2 と等しく、viewpoints の各行の id・name・reader・reads と question が folio2 の値に括弧の落としを当てた字と等しい。meta を持つ 5 file（srs・index・intake・ceiling・graph）の meta の approval が空の一覧。骨格の憲法の条は 1 本で id が P-1、その relations の rules が R-8 と R-16。adr/schema.yaml と design-note/schema.yaml の meta の decided_by が ADR-1 だけで、adr/ADR-1.yaml の status が proposed。**base では落ちる＝RED。**
...
4. **言えないこと — folio2 の正本との結びつき。** init は folio2 の 5 本を組み立て時に読み、歯 5 と歯 7 は folio2 の今の正本と突き合わせる。後の便で folio2 の入口・天井・相談窓口の写す節に、全角の括弧の外で folio2 の id を書くと、その便で歯 7 が落ちる（写しの漏れを拾う意図どおり・その便が式か字を直す）。全角の括弧の中に書くと括弧ごと落ちるので歯 7 は通り、歯 5 も folio2 の値に同じ落としを当てて比べるので通る（落ちた後の字が意味を失うかどうかは歯では言えず、天井か持ち主の読み）。括弧の落としで値の形が変わる場所（例 天井の観点の reads の一覧の字）に書くと、骨格の床か歯 1 が落ちうる（落ちた便が字の置き場を直す）。
...
- 前提の着地: 版 B（要件書 第 1.34 版・main 2d90e8d に在る）・便 ②・便 ③・便 123・一括 16（要件書 第 1.35 版・FR22 の規範文に graph.yaml・発効済み・本改訂 d でその字に揃えた・§0 の 前の便）。
...
- 本便の着地の後に席が見ること: 要件 FR22 の注の「口は未実装である」の字と、受入基準 AC19 / AC26 / AC27 の固定の材料の字（置き場は本便で決まった `tests/fixtures/schema/` の下の init-forbidden.txt と歯の file）を次の要件書の版で直す。入口の相談窓口の節の note に骨格の命令を添えること（天井 28 周目 整合 F-5・一括 16 では D）。
