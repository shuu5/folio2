...
- 前の便: **base = main 2d90e8d。この契約の数はすべて base 2d90e8d の実測（参考値）で、受付の時点の main で数え直す**（規則の表の行 D-13）。**受付の順序 = 便 ②（`docs/design/delivery-121.md`〔と 124〕・列の根の表と始まりの凍結）→ 便 ③（`docs/design/delivery-122.md`・値域の部分集合の床）→ 便 123 → 本便**（ADR-16 決定 (7) の ② → ③ → ④・版 B は main に在る）。骨格の生成区間は既存の `folio schema --write` の口（schema.rs の run）を呼んで書くので、便 ② が判断の記録の欄の決まりの生成区間を置き場の憲法の名で決まる形にすると、骨格もそのまま同じ形になる（§1 (c) の 1）。
...
- **書けない（2）。** 置き場が symlink か dir でない file・置き場の親 dir が無い・書けない・(2) の run が合格を返さない。途中で書けなくなったら、そこで止めて 2 で終わり、書いた file と書かなかった file の名を全部出す（消さない・N-1.1・黙って半分を合格にしない・P-4.1）。run が返した理由の字も標準エラーに出す。
- **書いた（0）。** 1 file 1 行で「folio init: 書いた <path>（<byte> byte）」と、最後に次の手の 1 行を出す: 版管理に add して commit してから `folio check --dir <置き場>` を撃つこと（commit の前に撃つと「commit が 1 つも無い」の「まだ分からない」が 1 件足される＝検証役の実測）と、凍結の基準は後で凍結すること（始まりの凍結の命令の旗の名は便 ② が決めるので出さない・§1 (i) の 5）。
...
- 本便の後の木で workspace の nextest は全部緑（**受付の時点の main の本数 + 本便の新しい歯 7**・起草役の参考値は base 2d90e8d で 796、便 123 の後で 803、便 ② ③ と便 123 の起草の patch を全部当てた木で 815）。
...
4. **言えないこと — folio2 の正本との結びつき。** init は folio2 の 5 本を組み立て時に読み、歯 5 と歯 7 は folio2 の今の正本と突き合わせる。後の便で folio2 の入口・天井・相談窓口の写す節に、全角の括弧の外で folio2 の id を書くと、その便で歯 7 が落ちる（写しの漏れを拾う意図どおり・その便が式か字を直す）。
...
- 前提の着地: 版 B（要件書 第 1.34 版・main 2d90e8d に在る）・便 ②・便 ③・便 123（§0 の 前の便）。
