...
4. **天井の正本が load で読めなければ面は まだ分からない（改訂 b・検証の低い重さ 3）。** 今の名札の読み手 findings::viewpoint_names は観点の id と名だけを読む緩い読みで、`crates/folio/src/ceiling_src.rs` の load は reads の行き先と所見の欄の決まり（load_rules）まで読む厳しい読みである。そこで歯 3 と同じ写しの作りで、観点 fidelity の reads に documents の一覧に無い行き先 `{doc: nowhere, fields: [x]}` を足す（viewpoint_names は通り、load は Err「行き先「nowhere」が一覧に無い」）→ `folio face --face index --write` が終了コード 2・出力の file を書かない・標準エラーに字「正本の要約値が測れない」と「nowhere」が在る。**base では同じ写しで 5 面とも終了コード 0 で書く（起草役の実測・改訂 b）＝RED。** base の門は同じ写しで「まだ分からない（ceiling.yaml: viewpoints[fidelity].reads: 行き先「nowhere」が一覧に無い）」を返す。この歯が無いと、load の Err を握りつぶして添え書きなしで出す変異が生き残る。
...
2. **余地（CapHeadroom）。** 測るのは `crates/folio/src/` の下の印なしの .rs の 2 本。測り方は各行の字数を 120 で割って切り上げ（空行は 1）、全行で足す（`wc -l` ではない・script は §1 (i)）。size S の見積は 1 file あたり 100。
   ...
   行の長さは Unicode の字で数える（起草役と検証役の値が一致した）。byte で数えると face.rs は 1,042（余地 458）・stamp.rs は 359（余地 1,141）で、どちらの数え方でも S の 100 の内である。
   2 本とも余地は S の 100 を超える。便アの着地で stamp.rs は約 345 になる見込み（便アの契約の §1 (g) の 2）で、余地は 1,100 を超えたまま。face.rs は便アの write-set に無い。歯の file は src の外なので余地を測らない（badge.rs は base 749 行・helper と歯 4 本で約 200 行増える見込み）。
...
8. 歯 3・4 の土台（改訂 b・便アの着地の後に要る）: `grep -c 'reads: \[{doc: srs, fields: \[requirements.plain\]}\]}' tests/fixtures/face/ceiling.yaml` が 1。face_copy の 9 file の写しに独立の物差しの値の印を置いて門が「通す（…正本の要約値が同じ）」（load を通る）。その写しで歯 3・4 の置き換えをして、base の binary の門がそれぞれ graph.yaml と nowhere の字で まだ分からない を返す。
