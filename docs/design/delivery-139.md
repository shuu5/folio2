...
- 改訂 c（2026-09-25・独立の検証 d139-verify.md〔条件付き支持・blocking 1・数値は B-1 のほかは全部一致〕）: blocking B-1 = 当たり判定 sc-hit が折りたたみと一覧を覆い最新の記録へ飛ぶという字は実測で偽（base の 4 枚とも 0×0 で効いていない）。席の裁定で外す決定は残し、理由を今は効いていない・様式の定義が意図どおりに直ったときに一覧の押した先を覆わないための先回り・今の見た目と押した先は不変、に直した（§1 (a) の 5・(b) の 2・4・6・(d) の 4・(i) の 2・契約表の title）。様式の定義の潜在の不具合は席が起票する台帳の控えとして §5 に書いた。任意の提案から、(f) の 2 に行数の上限の歯 schema_docs.rs の 700、(e) の 4 に変異 2 行（行き先を全部最新へ・畳まない・起草役の実装でも落ちることを実測）、(i) の 2 に開いた一覧の最長の行の高さを足した。数値・write-set 8・verify 6・done・size S は変えない。
...
5. **当たり判定は今は効いていない。** 様式の定義の意図（部品目録 parts.json の sc-hit = 棚カードの全面リンク）は card 全体を覆う当たり判定だが、生成器が sc-hit を class sc-row の行の中に置くため、行の中のリンクを上に出す規則（folio.css 177 行・詳細度が高い）が当たり判定の絶対配置の規則（176 行）に勝ち、4 枚の card とも当たり判定の大きさは 0×0 で効いていない（独立の検証役の base の実測）。card の右上の空きを押しても何も起きない。この様式の定義の潜在の不具合は本便の範囲の外で、席が起票する台帳の控えで扱う（§5）。
...
- 台帳の控え（席が起票する）: 入口の棚の card の当たり判定 sc-hit が、様式の定義の規則どうしの詳細度で 4 枚とも効いていないこと（§1 (a) の 5・folio.css は設計文書の置き場で門の対象・直すと入口のほかの 3 枚の操作が変わるので本便に入れない）。
