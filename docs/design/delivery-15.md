...
(e) 歯 `crates/folio/tests/face.rs`（既存の歯の file に足す・関数名はすべて face を含める＝verify の filter 語・便 14 の歯は本文も期待も変えない）。base で名に face を含む歯は `tests/face.rs` の 16 本・`src/face.rs` の unit 5 本・`src/face_constitution.rs` の unit 2 本・`tests/parts.rs` の 3 本で、`tests/parts.rs` は write-set に在るが本文も期待も変えない（実測: 2026-09-17・main ae9bd12）。便 0 の歯 `tests/check.rs`（5 本・(b0) の受け皿の回帰）は verify に `check` の filter で名指し、その filter が base で解く歯の file は `tests/check.rs` 5・`tests/gitcheck.rs` 4・`src/gitcheck.rs` 1・tests/parity.rs（2026-09-18 に scripts/retired/ へ退役） 1・`tests/render.rs` 1・`tests/parts.rs` 14・`tests/face.rs` 2（実測）で、`tests/gitcheck.rs`・`src/gitcheck.rs`・tests/parity.rs（2026-09-18 に scripts/retired/ へ退役）・`tests/render.rs` は write-set に在るが本文も期待も変えない。
