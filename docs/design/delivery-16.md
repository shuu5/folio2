...
(d) 歯 `crates/folio/tests/face.rs`（既存の歯の file に足す・関数名はすべて face を含める＝verify の filter 語・便 14・15 の歯は変えない）。filter 語 face が base で解く歯は `tests/face.rs` 30・`src/face.rs` 5・`src/face_constitution.rs` 2・`src/face_srs.rs` 5・`tests/parts.rs` 3（実測・main d747a9c）で、`src/face_constitution.rs`・`src/face_srs.rs`・`tests/parts.rs` は write-set に在るが本文も期待も変えない（`src/face_srs.rs` の幅 120 で正規化した行数は 1,370 で余地 130＝size は S で申告する）。
