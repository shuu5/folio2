...
判定と終了コードは便 2 の §1 のまま（合格 0 / 違反 1 / まだ分からない 2）。`crates/folio/src/inject.rs`（unit の歯の名が inject を含む）と `crates/folio/tests/parity.rs` は verify で回す歯の file として write-set に在るが触らない（本文も期待も変えない）。`crates/folio/src/` の実装は変えない。外部 crate は増やさない（`Cargo.toml` と `Cargo.lock` は触らない）。正規表現は使わない。CI は Python と pyyaml を持ち続ける（parity の歯が check_draft.py を回す）。
