...
5. 実の正本: design-intent の写し（copy_tree・本便の前段 = folio build --out で配信先を作る）から組む → 0 ∧ 4 dir ∧ 各 digest.txt を sha256sum で測り直して一致 ∧ fidelity の sources/adr/ に .yaml が 9 本（ADR-1〜8 と schema.yaml）・sources/design-note/ に 3 本・faces/ に adr- の面 8 本と note- の面 1 本。
