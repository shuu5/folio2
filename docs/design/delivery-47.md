...
5. crates/folio/tests/bundle.rs: 既存の歯 bundle_unknown_when_the_bundle_contents_differ_from_the_floor は、突き合わせが消えるので差し替える = fixture の天井の正本の写しから旧 4 節を外し末尾に anchor の中身を足しても、folio ceiling の書き出しが凍結 anchor の要約値 4 本と同じ束を組む（読み手が file の一覧に依らなくなったこと）。関数名は bundle で始める。要約値が変わらない根拠（設計判断の席の実測）= 束に写す正本は、その観点の reads が名指す文書だけである（bundle.rs の束を組む関数は reads の doc を順に documents に解いて写す）。この fixture tests/fixtures/ceiling/bundle/source/ceiling.yaml の 4 観点の reads が名指す doc は adr・constitution・design-note・index・rules・srs の 6 つで、天井の正本（doc の id は ceiling）を名指す reads は 1 つも無い。したがって fixture の天井の正本の file そのものはどの観点の束にも写されず、その byte を書き換えても 4 本の要約値は動かない。束の中の question.yaml・reads.yaml は書き換えない節（viewpoints）から組まれ、finding.yaml は (d) のとおり値が同じなので byte が同じである。もし作業者の実測でこの前提が崩れていたら（要約値が動いたら）、歯の期待を書き換えて合わせず、止めて設計判断の席へ問う。
