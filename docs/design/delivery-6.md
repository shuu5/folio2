...
(a) 撤退条件の種類の一致。床の定数（便 5 の (b)）の enums.retreat_kind と、憲法の schema 節の enums.retreat_kind を比べ、一覧の長さか要素の字面か順が違えば 1 違反（種別 adr・憲法が正）。憲法の schema.enums.retreat_kind が無いか一覧でなければ「まだ分からない」。
