...
- 要件: FR4（入口を含む 3 面を 1 つの生成器で出す）/ FR1（相談窓口の支度表を入口が見せる）
- 条: P-2.1 / P-12.2
...
(d) 凍結の面の写し。repo の根の tests/fixtures/face/expected-index-sheet.html（crates/folio/ の下ではない・歯は repo_root() から読む・支度表つきの入口の凍結 anchor）は (c) で変わるので、同じ着地で fixture の正本から生成し直して置き換える（手で直さない）。expected-index.html（支度表なし）は変わらない = 触らない。
...
3. lane_adr_stop_anchor_rejects_other_shapes: fixture の入口の正本の写しで、stop を {doc: adr, at: s1, label: x} にすると生成が Err で stderr に「行き先」を含む（ADR-n の形の外は断る・面を通す結合の歯 = binary は lib を持たないので tests 側からしか測れない）。
...
(f) 大きさと接続。新規 file は無い。crates/folio/src/face.rs（+約 5 行・余地 252 → 約 247）・crates/folio/src/face_index.rs（+約 30 行・余地 257 → 約 227）・crates/folio/tests/face_index.rs（+約 70 行）・repo の根の tests/fixtures/face/expected-index-sheet.html（再生成・write-set の 4 行目）。size S。index.yaml（読む順番に実際に判断の記録の行を足すのは席の一括）・parts.json は触らない。外部 crate は増やさない。
...
size = "S"
done = "lane_adr_ の歯 3 本（adr-2.html へ・無い id は断る・形の外は断る・3 本とも面を通す結合の歯）と sheet_approval_ の歯 1 本（承認済み（日付）・逐語は details の中だけ）が緑、tests/face_index.rs の既存の歯が全部緑（expected-index.html は不変・expected-index-sheet.html は再生成）、clippy が 0 警告で CI が通る"
<!-- contracts:end -->
