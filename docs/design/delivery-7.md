...
床の定数の所在: anchor に関わる床の定数（file_keys・first_version・root_digest・version_pattern・digest_algo・projection_article_fields・statement_fields・scope_minimum と印 3 つ）は便 5 が `crates/folio/src/adr.rs` に置いた非公開の定数 FLOOR の中に在り（便 6 が crate の中へ可視性を広げるか読み口を足す）、`anchor.rs` はそれを読む。便 6 が開いた可視性で足りなければ `adr.rs` の中で crate の中へ広げるか読み口の関数を足す（値と検査の式は変えない・adr.rs は write-set に在る・src の unit の歯の名に adr は無いので歯の門には掛からない）。違反の記録型は便 4〜便 6 と同じで、`verdict.rs` の Report の関数 violation は種別を文字列で受け、種別（anchor・A-2・N-4・P-7・adr）は文字列 1 つで閉じる。
