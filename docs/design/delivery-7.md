...
便 0〜便 6 の fixture 14 組（`tests/fixtures/check/` の 3 組・`refs/` 3 組・`vocab/` 2 組・`adr/` 3 組・`link/` 3 組）は anchors/ を持たず記録も無いので、(i) で 測れない（pending）が 1 つ立つ。(a) の並びにより、違反 1 件の組は 不合格（終了コード 1）のまま・要約行の「まだ分からない」が 0 から 1 になるだけで、各歯（`crates/folio/tests/check.rs`・`refs.rs`・`vocab.rs`・`adr.rs`・`link.rs`）の期待（終了コード・違反の行数・種別と文言）は変わらない。`check/missing-file/` は 4 file の読みで止まり 2 のまま。これら 5 本の歯は verify で回すために write-set に在るが触らない（本文も期待も変えない）。fixture 14 組も触らない。
