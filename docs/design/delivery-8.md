...
突き合わせの歯（crates/folio/tests/parity.rs（2026-09-18 に scripts/retired/ へ退役）・便 7 の 27 入力に足す・写し全部 + git 1 commit + 変異 1 つ・床と folio の終了コードの一致）: (28) 写しの `anchors/` を丸ごと消す → 1（HEAD にあるが無い）／(29) 写しの `anchors/constitution-v1.0.yaml` だけを消す（索引は残す）→ 1／(30) 写しの版管理の根に `.gitignore` を作り anchors/ を書く → 1／(31) 写しの `.git` を消す → 2（版管理が無い＝測れない・違反 0）。既存の 27 入力は変えない。
