...
2. 回帰（期待不変・verify の 2 行目と共通の検証）: crates/folio/tests/face.rs・tests/face_constitution.rs・tests/face_srs.rs・tests/face_index.rs・tests/face_adr.rs・tests/face_note.rs・tests/site.rs・tests/badge.rs・tests/parts.rs の既存の歯すべてと、face.rs の単体の歯 mod face_tests の 9 本（名札の凍結の針を含む）。凍結の写し 7 本との byte 一致がここで見られる。
