...
- **憲法の面の数値の表で、裁定の枡が最新の裁定を 2 件つなげて出す行が在る**（28 周目 読みやすさ F-7 の面の側。語彙の「一括」の項は一括 16 で足した）— `crates/folio/src/face_constitution.rs` の `ruling` は裁定の欄を字「）・前の裁定 = 」で割り、先頭の 1 件を枡に出して残りを小窓「前の裁定 N 件」へ畳む。規則の表の 11 行（R-2・R-3・R-5・R-8・R-15・D-1・D-2・D-3・D-4・D-7・D-10）は、裁定と裁定の間の一部がこの区切りの字でなく「）・」だけでつながっている（一括 11 が呼び名を統一したときの頭の 1 件の後など）。そのため枡に 2〜3 件が出て、持ち主がいまの裁定を探すと来歴に埋もれる。さらに、並びが新しい順でない行が在る（行 R-5 は先頭が「申告 B（初版 2026-09-12）」で後ろの「ADR-4 発効承認 2026-09-16」のほうが新しい・行 D-1 は 09-22・09-12・09-21 の順）。項の頭の形も一様でない（「f2-648 notes と時刻」「G の番号」「発効承認」のほかに「持ち主 2026-09-12 17:1x 裁定」〔D-1・D-2・D-10〕・「依存を足す A-3.1 の承認」〔R-15〕・「初版 = 運用実績」〔R-8〕）。直し方は 2 つ在り、便の設計で決める。(a) 面の側で、項の境目（「）・」の後に日付か裁定の名を持つ項が始まる所）も区切りに数え、項を日付で並べ替えて最新の 1 件を枡に出す（並べ替えは区切りだけでは解けない R-5・D-1 のため・日付の無い項〔G の番号だけの項〕の置き方も便で決める）。(b) 正本の側で区切りの字を揃える。(b) は規則の表の行の変更なので、行ごとに裁定 id と時刻が要り（条 P-17.1）、裁定の欄の頭に「区切りを揃えただけ」の裁定がもう 1 件足される。**推奨は (a)**（正本を動かさず、来歴を増やさない）。(b) を選んでも R-5・D-1 の並びは正本の行の変更なしには直らない。歯は `crates/folio/tests/face_constitution.rs` が裁定の枡の字を pin しているかを便の起草で確かめる。余地（§3 の式・2026-09-24・main 2d90e8d）は `crates/folio/src/face_constitution.rs` 461 で、S でも M でも受付の余地がある。
- **参考（28 周目 読みやすさ F-9・一括 16 は D）: 入口の棚の要件書のカードと要件書の表紙が、最初の作成日（`meta.generated`）を「更新」の名札で出す。** 同じ表紙の状態の欄は承認の日付を出すので、読み手は文書がいつ変わったかを取り違える（憲法のカードも同じ形）。直す先は面の生成器の名札（「作成」にするか、承認欄の最後の日付を「更新」に出すか）。上の裁定の枡の便か §13 の判断の記録のカードの便を起こすときに、同じ便に入れるかを便の設計で決める（一括 16 では起票しない）。
