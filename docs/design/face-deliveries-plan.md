...
天井の 28 周目の所見のうち、直す先が面の生成器（実装）だったもの。生成区間（床の定数の説明の注）の便候補 3 本は面の便ではないので、ここではなく `docs/design/batch16-triage.md` の仕分けの表と「この一括に入れなかったもの」に置いた。

- **憲法の面の数値の表で、裁定の枡が最新の裁定を 2 件つなげて出す行が在る**（28 周目 読みやすさ F-7 の面の側。語彙の「一括」の項は一括 16 で足した）— `crates/folio/src/face_constitution.rs` の `ruling` は裁定の欄を字「）・前の裁定 = 」で割り、先頭の 1 件を枡に出して残りを小窓「前の裁定 N 件」へ畳む。規則の表の 11 行（R-2・R-3・R-5・R-8・R-15・D-1・D-2・D-3・D-4・D-7・D-10）は、裁定と裁定の間の一部がこの区切りの字でなく「）・」だけでつながっている（一括 11 が呼び名を統一したときの頭の 1 件の後など）。そのため枡に 2〜3 件が出て、持ち主がいまの裁定を探すと来歴に埋もれる。直し方は 2 つ在り、便の設計で決める。(a) 面の側で、裁定 id の形（f2-648 notes と時刻・G の番号・発効承認）で始まる項の境目も区切りに数える。(b) 正本の側で区切りの字を揃える。(b) は規則の表の行の変更なので、行ごとに裁定 id と時刻が要り（条 P-17.1）、裁定の欄の頭に「区切りを揃えただけ」の裁定がもう 1 件足される。**推奨は (a)**（正本を動かさず、来歴を増やさない）。歯は `crates/folio/tests/face_constitution.rs` が裁定の枡の字を pin しているかを便の起草で確かめる。余地（§3 の式・2026-09-24・main 2d90e8d）は `crates/folio/src/face_constitution.rs` 461 で、S でも M でも受付の余地がある。
